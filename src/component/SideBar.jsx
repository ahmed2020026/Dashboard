...
export const SideBar = () => {
    ...
    return (
        <>
            <div className={`bg-white p-4 fixed w-60 shadow-xl z-10 md:shadow-none text-[14px] font-medium h-full ${openMenu ? '-left-full' : 'left-0'} duration-300 md:left-0`} ref={SidebarEle}>
                <ul className='text-gray-800'>
                    {
                        routesLinks.map((item, index) => (
                            !item.subRoute ? (
                                <li key={index} className='bg-gray-100 py-2 rounded-md mb-1 flex font-medium items-center px-2 gap-2'>
                                ...
                                <li key={index} className={`mb-1 overflow-hidden duration-200`} style={{ height: current == index ? `${height}px` : '40px' }} ref={current == index ? ele : null}>
                                    <button className='bg-gray-100 py-2 w-full cursor-pointer rounded-md flex items-center gap-2 px-2 relative' onClick={(e) => toggle(index)} >
